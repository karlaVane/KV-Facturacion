from Facturacion.models import Contribuyente, Detalle_pedido, Pedido, Consumidor
...
class Woocommerce:
    ...
    def get_pedidos(self, after):
        url = 'orders/?per_page=2&after={}&order=asc'.format(after)
...
class Clase_Pedidos:
    ...
    ## --------------------------------------- Tabla Pedido ----------------------------------
    #def guardar_pedidos(self,pedido,cedula,estado,href,json_completo,total,t_imp):
    def guardar_pedidos(self,pedido,cedula,estado,href,total,t_imp):
    ...
    def guardar_todo(self,fecha):
        ...
        for pedido in pedidos: 
            ...
            if (Consumidor.objects.filter(identificacion = cedula_consumidor).exists()): #Comprueba existencia del Consumidor
                self.guardar_pedidos(pedido['id'],cedula_consumidor,pedido['status'],pedido['_links']['self'][0]['href'],pedido['total'],pedido['total_tax'])
            else:
